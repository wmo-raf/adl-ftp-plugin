...
RUN groupmod -g $PLUGIN_BUILD_GID wis2box_adl_docker_group && usermod -u $PLUGIN_BUILD_UID $DOCKER_USER

# Install your dev dependencies manually.
COPY --chown=$PLUGIN_BUILD_UID:$PLUGIN_BUILD_GID ./plugins/adl_ftp_plugin/requirements/dev.txt /tmp/plugin-dev-requirements.txt
RUN . /wis2box_adl/venv/bin/activate && pip3 install -r /tmp/plugin-dev-requirements.txt

COPY --chown=$PLUGIN_BUILD_UID:$PLUGIN_BUILD_GID ./plugins/adl_ftp_plugin/ $WIS2BOX_ADL_PLUGIN_DIR/adl_ftp_plugin/
RUN . /wis2box_adl/venv/bin/activate && /wis2box_adl/plugins/install_plugin.sh --folder $WIS2BOX_ADL_PLUGIN_DIR/adl_ftp_plugin --dev
