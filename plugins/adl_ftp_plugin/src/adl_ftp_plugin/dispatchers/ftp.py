...
def has_valid_data(values: dict) -> bool:
    ...
    Returns:
        True if at least one value is not None, not an empty string, and
        not just whitespace
    """
    return any(
...
def prepare_csv_files_new_mode(
    ...
    Args:
        ...
        use_single_timestamp: If True, use a single 'timestamp' column in UTC
...
def prepare_csv_files_append_mode(
    ...
    for station_id, days in grouped.items():
        for day, incoming_records in days.items():
            # station_id here is actually the id_for_path (wigos_id, or
            # station_id as fallback)
            ...
            try:
                ...
                # Handle both FTP and SFTP errors, plus any other
...
def dispatch_to_ftp(channel, data_records: List[Dict], create_station_dir=True, include_wigos_id=True,
    ...
    Works with both FTP and SFTP connections automatically, based on the
    ...
    Args:
        ...
        include_header: Whether to include the header row in CSV files
    """
    # Use the flexible client method that returns an FTP or SFTP client based
