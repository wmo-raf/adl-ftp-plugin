...
from adl.core.exceptions import InstanceTypeDoesNotExist
...
def test_decoder_config(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                # The decoder bound to this connection's configuration — the
                # same resolution ingestion runs use, so what the page shows is
                # what a run would decode
                try:
                ...
                if configured is None:
                    error = error or f"Decoder '{decoder_name}' selected but no CSV configuration set"
                else:
                    ...
                    if values:
                        ...
                        parsed_data = {
                            ...
                            'decoder_name': configured.display_name,
                            'header': header,
                            'metadata': metadata,
                        ...
                        if configured.config is not None:
                            parsed_data['csv_config_name'] = configured.config.name

                        messages.success(
                            request,
                            f"Successfully parsed {len(values)} records from the file using {configured.display_name} decoder"
                        )
                    else:
